    ...
    for i, sample in enumerate(pbar):
        ...
        if np.random.random() < 0.025:
            if only_test:
                ...
                savefig = True
            denormalized = transforms.Normalize(mean=(0, 0, 0), std=(
            1 / 0.229, 1 / 0.224, 1 / 0.225))(sample[0].cpu())
            denormalized = transforms.Normalize(mean=(-0.485, -0.456, -0.406),
                                              std=(1, 1, 1))(denormalized)
            show_maps(denormalized, maps, loc_x, loc_y, epoch, model_name, savefig)
...
def main():
    ...
    basenet = VisionTransformer(embed_dim=384, patch_size=8, qkv_bias=True)
    basenet.load_state_dict(torch.load('./dino_deitsmall8_pretrain.pth'))
    ...
    dataset_val = CUBDataset(args.data_path, train_samples=dataset_train.trainsamples,
                             mode='val', transform=test_transforms)
