...
def landmarks_to_rgb(maps):
    ...
    # colors = [[0.25, 0, 0.25], [0.75,0, 0],[0,0.75,0],[0,0,0.75],[0.5,0.5,0],[0,0.5,0.5],[0.75,0.25,0],[0.75,0,0.25],[0,0.75,0.25],
    #
    ...
    for c in range(3):
        for h in range(len(maps)):
            for w in range(len(maps)):
                # if maps[h, w]==4:
                #     continue
                    # print(colors[maps[h, w]][c])
                rgb[h, w, c] += colors[maps[h, w]][c] * 2

                # rgb[:,:,c] += maps[:,:]*colors[m][c]
...
def plot_assignment(root, assign_hard, num_parts):
    ...
    colors = [[0.25, 0, 0.25], [0.75, 0, 0], [0, 0.75, 0], [0, 0, 0.75],
          ...
          [0.75, 0.25, 0.5], [0.25, 0.5, 0.75]]
    # colors = [[0.25, 0, 0.25], [0.75, 0, 0], [0, 0.75, 0], [0, 0, 0.75],
    #           [0.5, 0.5, 0], [0, 0.5, 0.5], [0.75, 0.25, 0], [0.75, 0, 0.25],
    #           [0, 0.75, 0.25],
    ...
    # ax.scatter(loc_y[0, :].detach().cpu() * 256 / assign_hard.shape[-1],
    #            loc_x[0, :].detach().cpu() * 256 / assign_hard.shape[-1],
    #            c='black', marker='x')
        # i += 1
...
def main():
    ...
    dataset = CUB200(root='../datasets/cub', train=False, transform=test_transforms)
    ...
    num_landmarks = 4
    model_name_init = '4parts_all_losses_normal_ABLATION_INDIVIDUAL.pt'
    ...
    with torch.no_grad():
        ...
        for col_id in range(fig_cols):
            for j in range(fig_rows):
                ...
                # inference the model
                input, target, _, _, path = next(test_loader_iter)
                ...
                save_input = torch.nn.functional.interpolate(input.data[0].unsqueeze(0), size=(448, 448), mode='bilinear', align_corners=False).squeeze(0)
