...
experiment = "cub_resnet_testnofc"
...
def train(net: torch.nn.Module, train_loader: torch.utils.data.DataLoader, device: torch.device, model_name: str,epoch: int, epoch_leftoff, all_losses: list = None):
    ...
    if all_losses:
        ...
        running_loss_conc, running_loss_mean, running_loss_max, running_loss_class, running_loss_equiv, running_loss_orth = all_losses
    ...
    optimizer = torch.optim.Adam(
            [{'params': list(net.parameters())[:-1], 'lr': 1e-4},
             {'params': list(net.parameters())[-1:], 'lr': 1e-3}])
    # optimizer = torch.optim.Adam(
    ...
    for i in range(len(train_loader)):
        ...
        loss_class = classif_loss(classif_anchor, lab.to(device))
        # loss_class_landmarks = classif_loss(scores_anchor.mean(-1), lab.to(device)) / 2
        ...
        pbar.set_description(
            "Cnc: %.3f, M: %.3f, Cls: %.3f, Eq: %.3f, Or: %.3f" % (
                running_loss_conc,
    ...
    all_losses = running_loss_conc, running_loss_mean, running_loss_max, running_loss_class, running_loss_equiv, running_loss_orth
    with open(f'./results_{experiment}/res.txt', 'a') as fopen:
        fopen.write(f'Epoch: {epoch}\n')
        fopen.write("Cnc: %.3f, M: %.3f, Cls: %.3f, Eq: %.3f, Or: %.3f\n" % (
                running_loss_conc,
                running_loss_max, running_loss_class, running_loss_equiv, running_loss_orth))
