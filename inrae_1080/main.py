...
def train(net: torch.nn.Module, train_loader: torch.utils.data.DataLoader, device: torch.device, model_name: str,epoch: int, epoch_leftoff, all_losses: list = None):
    ...
    for i in range(len(train_loader)):
        ...
        loss_class = classif_loss(scores_anchor, lab.to(device))
        ...
        for j in range(scores_anchor.shape[0]):
            sorted_classification, sorted_indices = scores_anchor[j, :].softmax(0).sort(descending=True)
        ...
        pbar.set_description("Cls: %.3f" % running_loss_class)
